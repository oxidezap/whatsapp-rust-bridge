...
//!
//! Port of the old wasm `src/client_profile.rs` with the tsify/wasm-bindgen
//! layer removed; serde derives/attrs (`tag`, `rename_all`, `flatten`,
//! `default`) and the `From` conversions are kept verbatim so JS inputs
//! deserialize identically via `serde_json`.
