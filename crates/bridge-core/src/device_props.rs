...
//! reused from prost) because we want a stable typed surface that doesn't churn
//! whenever the underlying proto adds a field.
//!
//! Port of the old wasm `src/device_props.rs` with the tsify/wasm-bindgen layer
//! removed; serde derives/attrs and all `From`/merge conversions are kept
//! verbatim so JS inputs deserialize identically via `serde_json`.
