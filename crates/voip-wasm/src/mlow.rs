...
pub use wacore::voip::rtp::RTP_PAYLOAD_TYPE_MLOW_RED;
        ...
        let pt: Option<u8> = if payload_type.is_undefined() || payload_type.is_null() {
            ...
            if n.is_finite() && n.fract() == 0.0 && (0.0..=255.0).contains(&n) {
                ...
                return Err(JsValue::from_str(&format!(
                    "payloadType: expected RTP payload type integer between 0 and 255, got {n}"
                )));
            }
        } else {
            return Err(JsValue::from_str(
                "payloadType: expected RTP payload type number",
            ));
        };
        self.inner
...
// JS values exist only in the wasm test target; Bun exercises the published
// decoder, including its payload-type boundary, in tests/voip-loader.test.ts.
